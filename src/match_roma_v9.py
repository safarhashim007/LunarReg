...
# +
# LOCKED robust weighted similarity refinement
# +
# constrained affine micro-refinement
#
...
#       ↓
# V8 coarse similarity transform
#       ↓
# V9 constrained affine micro-refinement
#
...
# ---------------------------------------------------------
# V9 affine micro-refinement.  These bounds are deliberately
# small: the affine stage may correct residual map anisotropy,
# but must not choose a different correspondence mode.
# ---------------------------------------------------------

AFFINE_REFINEMENT_THRESHOLDS = [7.0, 6.0, 5.0, 4.0]
AFFINE_MIN_CONSENSUS = 30
AFFINE_ALPHA = 0.55
MAX_AFFINE_ANISOTROPY = 0.045      # singular-value ratio - 1
MAX_AFFINE_SHEAR = 0.012           # normalized off-axis symmetric term
MAX_AFFINE_ROTATION_CHANGE = 1.25  # degrees from V8 coarse angle
MAX_AFFINE_TRANSLATION_CHANGE = 6.0  # output pixels from V8 transform
...
# =========================================================
# CONSTRAINED AFFINE MICRO-REFINEMENT (V9)
#
# The fit starts from the V8 mode and uses only its consensus.
# A candidate is projected back into a small, physically plausible
# neighbourhood of the coarse transform before it can be accepted.
# =========================================================
...
# V9 CONSTRAINED AFFINE MICRO-REFINEMENT
#
# The source pool is frozen from V8-consistent correspondences.
# Subsequent iterations can only remove points, never re-admit a
# match from a different geometric mode.
# =========================================================
...
for iteration, threshold in enumerate(AFFINE_REFINEMENT_THRESHOLDS, start=1):
    ...
    affine_refinement_log.append([iteration, threshold, int(new_mask.sum()), coverage,
                                  accepted, sx_aff, sy_aff, anis_aff, shear_aff,
                                  rot_aff, M_affine[0, 2], M_affine[1, 2],
                                  translation_delta, median_res])

# The V9 final matrix is affine. Keep the old variable name so all
# existing V8 diagnostics and visual generation evaluate the V9 result.
...
with open(
    f"{OUT}/roma_v9_similarity_refinement.csv",
    "w",
    newline=""
) as csvfile:
...
with open(f"{OUT}/roma_v9_affine_refinement.csv", "w", newline="") as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["iteration", "threshold", "consensus", "coverage", "accepted",
                     "axis_x", "axis_y", "anisotropy", "shear", "polar_rotation",
                     "tx", "ty", "translation_delta_from_v8", "median_residual"])
    writer.writerows(affine_refinement_log)
...
np.save(
    f"{OUT}/roma_v9_transform.npy",
    M_final
)

np.save(
    f"{OUT}/roma_v9_strict_A.npy",
    strict_A
)

np.save(
    f"{OUT}/roma_v9_strict_B.npy",
    strict_B
)

np.save(
    f"{OUT}/roma_v9_relaxed_A.npy",
    relaxed_A
)

np.save(
    f"{OUT}/roma_v9_relaxed_B.npy",
    relaxed_B
)
